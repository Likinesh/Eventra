...
const Explore = () => {
  ...
  const { data: featuredEvents, isLoading: loading } = useConvexQuery(
  ...
  const { data: categoryCounts } = useConvexQuery(api.events.getCategoryCount);
  ...
  const isLoading = loading || localLoading || popularLoading;
  if (isLoading) {
    return (
